...
def get_services():
    ...
    return {
        ...
        "streamlit": {
            "command": f"poetry run streamlit run src/italianollama/frontend/streamlit/app.py --server.port {streamlit_port}",
            "port": streamlit_port,
            "keywords": ["streamlit", "streamlit/app.py"],
            "env": {
...
def start_service(name):
    ...
    if proc:
        ...
        click.secho(f"⚠ Service '{name}' is already provided by a {status_text} process on port {services[name]['port']}.", fg="yellow")
        return
    ...
    click.secho(f"⚠ {name} started (PID: {process.pid}), but port {svc['port']} is not yet listening.", fg="yellow")
...
def stop_service(name):
    ...
    if proc:
        if is_tunnel:
            click.secho(f"⚠ Cannot stop '{name}': it is a REMOTE (TUNNEL) process (PID: {proc.pid}).", fg="yellow")
            return
...
def get_status():
    ...
    for name, info in services.items():
        ...
        if proc:
            status = click.style("REMOTE (TUNNEL)", fg="blue") if is_tunnel else click.style("RUNNING", fg="green")
